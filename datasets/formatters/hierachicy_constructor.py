    ...
    def greedy_partition_corrected(self, graph, max_cluster_size):
        ...
        while nodes_to_visit:
            ...
            while frontier and len(cluster) < max_cluster_size:
                ...
                cluster.append(node)
                frontier.extend(
                    [neigh for neigh in graph.neighbors(node) if neigh in nodes_to_visit and neigh not in frontier])
                frontier.remove(node)
                nodes_to_visit.discard(
                    node)  # Ensure nodes are removed from nodes_to_visit once they're added to a cluster
